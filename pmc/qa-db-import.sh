...
if [ "" == "$1" ] || [ "" == "$2" ]
then
	echo 'syntax: qa-db-import.sh <site-slug> <sql-file>'
	exit
fi
...
then
	...
	/usr/bin/wp --path=/srv/www/wp --require=/srv/pmc/pmc-wp-cli.php  pmc-site fix "${site_slug}.${DOMAIN}" --title="${site_slug}" --home="http://qa.{site_slug}.com"
	sudo service memcached restart
