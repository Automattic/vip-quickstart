...
then
	...
	sed -e "s/ INTO \`wp_/ INTO \`wp_${site_id}_/g" -e "s/CREATE TABLE \`wp_/CREATE TABLE \`wp_${site_id}_/g" -e "s/DROP TABLE IF EXISTS \`wp_/DROP TABLE IF EXISTS \`wp_${site_id}_/g" ${sql_file} | mysql -uroot wordpress
