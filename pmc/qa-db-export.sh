...
if [ "" == "$1" ] || [ "" == "$2" ]
then
	echo 'syntax: qa-db-export.sh <site-slug> <sql-file>'
	exit
fi
...
site_id=`/usr/bin/wp --path=/srv/www/wp site list --fields=blog_id,domain --format=csv|grep "${site_slug}.qa.pmc.com" | cut -d',' -f1`
